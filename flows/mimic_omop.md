...
| `use_trex_connection` | `True` | `True` exports through the trex pgwire passthrough (works for both Postgres and HANA). `False` uses the legacy path (DuckDB `ATTACH` for Postgres, SQLAlchemy chunked inserts for HANA). |
